<script>
	$(function () {
		const table = $("#table").DataTable({
			processing: true,
			serverSide: true,
			ajax: "<?php echo e(route('api.v1.datatables.customer.index')); ?>",
			columns: [
        ...
        whatsapp: $("#createModal form #whatsapp").val(),
			...
			$.ajax({
				url: "<?php echo e(route('api.v1.datatables.supplier.store')); ?>",
				method: "POST",
				header: {
		...
		$("#table").on("click", ".update-modal", function () {
			...
			let url =
				"<?php echo e(route('api.v1.datatables.supplier.show', ':paramID')); ?>".replace(
					":paramID",
					id
				);
			let updateURL =
				"<?php echo e(route('api.v1.datatables.supplier.update', ':paramID')); ?>".replace(
					":paramID",
					id
				);
			...
			$.ajax({
				...
				success: (res) => {
					...
					$("#updateModal form #nomer_pajak").val(res.data.npwp);
		...
		$("#updateModal form").submit(function (e) {
			...
			const formData = {
				...
				nomer_pajak: $("#updateModal form #npwp").val(),
			...
			$.ajax({
				...
				success: (res) => {
					...
					Swal.fire({
						icon: "success",
						title: "Data customer berhasil diubah!",
						toast: true,
						position: "top-end",
						showConfirmButton: false,
						timer: 3000,
...
<?php /**PATH C:\Users\Unknown\OneDrive\Documents\KERJA\FREELANCE\sph\resources\views/customer/script.blade.php ENDPATH**/ ?>