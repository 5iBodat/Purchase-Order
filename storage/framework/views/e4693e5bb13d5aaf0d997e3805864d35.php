<script>
	$(function () {
		...
		const table = $("#table").DataTable({
			processing: true,
			serverSide: true,
			ajax: "<?php echo e(route('api.v1.datatables.cash-transactions.index')); ?>",
			columns: [
		...
		$("#createModal form").submit(function (e) {
			...
			$.ajax({
				url: "<?php echo e(route('api.v1.datatables.cash-transactions.store')); ?>",
				method: "POST",
				header: {
		...
		$("#table").on("click", ".show-modal", function () {
			...
			let url =
				"<?php echo e(route('api.v1.datatables.cash-transactions.show', ':paramID')); ?>".replace(
					":paramID",
					id
				);
		...
		$("#table").on("click", ".update-modal", function () {
			...
			let updateURL =
				"<?php echo e(route('api.v1.datatables.cash-transactions.update', ':paramID')); ?>".replace(
					":paramID",
					id
				);
		...
		$("#table").on("click", ".delete", function (e) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					const id = $(this).data("id");
					const url =
						"<?php echo e(route('api.v1.datatables.cash-transactions.destroy', ':paramID')); ?>".replace(
							":paramID",
							id
						);
...
<?php /**PATH /Users/nugihiday/Documents/Workspace/sph/resources/views/cash_transactions/script.blade.php ENDPATH**/ ?>