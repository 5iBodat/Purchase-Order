<script>
	$(function () {
		const table = $("#table").DataTable({
			processing: true,
			serverSide: true,
			ajax: "<?php echo e(route('api.v1.datatables.po-transport.index')); ?>",
			columns: [
		...
		function totalData(){
			$.ajax({
				url: "<?php echo e(route('api.v1.datatables.po-transport.get-total')); ?>",
				method: 'POST',
		...
		$("#table").on("click", ".show-modal", function () {
			const id = $(this).data("id");
			let url =
				"<?php echo e(route('api.v1.datatables.po-transport.show', ':paramID')); ?>".replace(
					":paramID",
					id
				);
		...
		$("#table").on("click", ".update-modal", function () {
			...
			let url =
				...
				let updateURL =
				"<?php echo e(route('api.v1.datatables.po-transport.update', ':paramID')); ?>".replace(
					":paramID",
					id
				);
		...
		$("#table").on("click", ".download", function(e) {
			e.preventDefault();
			const id = $(this).data("id");
			const url = "<?php echo e(route('api.v1.datatables.po-transport.download')); ?>";
			$.ajax({
				url:url,
				method: "POST",
				data: {id:id},
		...
		$("#createModal form").submit(function (e) {
			...
			$.ajax({
				url: "<?php echo e(route('api.v1.datatables.po-transport.store')); ?>",
				method: "POST",
				header: {
					"Content-Type": "application/json",
				},
		...
		type.addEventListener('change', function(e) {
			...
			$.ajax({
				url: "<?php echo e(route('api.v1.supplier-setting',':id')); ?>".replace(':id', id),
				method: "GET",
				header: {
					"Content-Type": "application/json",
				},
		...
		to.addEventListener('change', function(e) {
			...
			$.ajax({
				url: "<?php echo e(route('api.v1.supplier-setting.generate-po-number')); ?>",
				method: "POST",
				header: {
					"Content-Type": "application/json",
				},
...
<?php /**PATH C:\Users\Unknown\OneDrive\Documents\KERJA\FREELANCE\sph\resources\views/po_transporter/script.blade.php ENDPATH**/ ?>