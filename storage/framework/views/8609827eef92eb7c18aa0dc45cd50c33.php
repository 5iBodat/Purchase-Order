<script>
	$(function () {
		const table = $("#table").DataTable({
			processing: true,
			serverSide: true,
			ajax: "<?php echo e(route('api.v1.datatables.administrators.index')); ?>",
			columns: [
		...
		$("#createModal form").submit(function (e) {
			...
			$.ajax({
				url: "<?php echo e(route('api.v1.datatables.administrators.store')); ?>",
				method: "POST",
				header: {
		...
		$("#table").on("click", ".show-modal", function () {
			...
			let url =
				"<?php echo e(route('api.v1.datatables.administrators.show', ':paramID')); ?>".replace(
					":paramID",
					id
				);
...
<?php /**PATH C:\Users\Unknown\OneDrive\Documents\KERJA\FREELANCE\sph\resources\views/administrators/script.blade.php ENDPATH**/ ?>