<script>
	$(function () {
		const table = $("#table").DataTable({
			processing: true,
			serverSide: true,
			ajax: "<?php echo e(route('api.v1.datatables.school-classes.index')); ?>",
			columns: [
		...
		$("#createModal form").submit(function (e) {
			...
			$.ajax({
				url: "<?php echo e(route('api.v1.datatables.school-classes.store')); ?>",
				method: "POST",
				header: {
		...
		$("#table").on("click", ".show-modal", function () {
			...
			let url =
				"<?php echo e(route('api.v1.datatables.school-classes.show', ':paramID')); ?>".replace(
					":paramID",
					id
				);
		...
		$("#table").on("click", ".update-modal", function () {
			...
			let updateURL =
				"<?php echo e(route('api.v1.datatables.school-classes.update', ':paramID')); ?>".replace(
					":paramID",
					id
				);
		...
		$("#table").on("click", ".delete", function (e) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					const id = $(this).data("id");
					const url =
						"<?php echo e(route('api.v1.datatables.school-classes.destroy', ':paramID')); ?>".replace(
							":paramID",
							id
						);
...
<?php /**PATH C:\Users\Unknown\OneDrive\Documents\KERJA\FREELANCE\sph\resources\views/school_classes/script.blade.php ENDPATH**/ ?>