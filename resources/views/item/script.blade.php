<script>
	$(function () {
		const table = $("#table").DataTable({
			processing: true,
			serverSide: true,
			ajax: "{{ route('api.v1.datatables.item.index') }}",
			columns: [
		...
		$("#createModal form").submit(function (e) {
			...
			$.ajax({
				url: "{{ route('api.v1.datatables.item.store') }}",
				method: "POST",
				header: {
		...
		$("#table").on("click", ".update-modal", function () {
			...
			let url =
				"{{ route('api.v1.datatables.item.show', ':paramID') }}".replace(
					":paramID",
					id
				);
			let updateURL =
				"{{ route('api.v1.datatables.item.update', ':paramID') }}".replace(
					":paramID",
					id
				);
		...
		$("#table").on("click", ".delete", function (e) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					const id = $(this).data("id");
					const url =
						"{{ route('api.v1.datatables.item.destroy', ':paramID') }}".replace(
							":paramID",
							id
						);
